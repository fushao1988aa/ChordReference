...
一、总体架构
ChordReference 是一个和弦调性分析与创作辅助插件，主要面向音乐制作人、作曲者。它既能实时识别MIDI输入的和弦，又能作为智能和弦进行预设浏览器，还能生成上下文感知的分解琶音，并正在开发自动旋律生成功能。
...
构建过程
buildChordDatabase() 为15个大调主音（包括等音）构建：
...
实时识别流程
MIDI输入回调：记录按下的音符、力度、延音踏板。

防抖机制：音符集合稳定75ms后才触发识别。
...
输出结果：MidiChordResult 包含调性、调式、功能名（如 T7）、MIDI音符、力度等。

触发回调：newResultAvailable 通知 UI 线程刷新。

试听系统
内置钢琴采样器（7层，C2-C8），带混响、高通/峰值/高架滤波、软限幅。
...
这是项目中代码量最大的UI组件（约3000行），实现了完整的预设管理系统。
...
ArpeggioMatcher
核心匹配引擎，根据 ArpeggioContext 对每个范式进行多维度打分：
...
过期后显示覆盖层，阻止使用（但代码中似乎未完全强制禁用，仅UI提示）。
...
模块化清晰：每个功能域独立成类/文件，职责单一。

性能优化：和弦识别有防抖；标签统计有缓存；MIDI设备列表按需刷新；UI缩放避免频繁重绘。

可扩展性强：添加新琶音范式只需在 ArpeggioPatternLibrary 末尾追加，匹配器自动参与评分。

鲁棒性：大量 try-catch 保护和弦构建失败；文件操作有错误检查；空指针判断。

用户体验丰富：情绪文字、封面图片、拖拽生成、实时反馈、多语言、多皮肤。
...
- 使用 JUCE 抽象层，路径处理、字体降级均有 Windows/macOS/Linux 分支。
- 中文显示实现要点总结**
...
通过 CMake 的 juce\_add\_binary\_data 嵌入为二进制资源

在代码中通过 BinaryData 加载并设置为默认字体

**2.源文件编码必须为 UTF-8 with BOM**

每个包含中文字符串的 .cpp / .h 文件，在 Visual Studio 中必须另存为 “Unicode (UTF-8 带签名) - 代码页 65001”

不带 BOM 的 UTF-8 在 MSVC 下可能被误判为 GBK

**3.编译选项强制 UTF-8 + 字符串字面量使用 u8 前缀**

CMake 中添加：target\_compile\_options(ChordReference PRIVATE /utf-8)（MSVC 专用）

所有中文字符串使用 u8"中文" 语法，或通过 juce::String::fromUTF8(u8"...") 包裹

这确保编译器生成的二进制数据是正确的 UTF-8 编码
...
- - **预设浏览系统新增进行分解与分解进行拖拽到轨道**：强大的分解范式库，智能Top评分生成机制
-  - **预设浏览系统新增和弦进行预设库到700+**：新增了大量和弦进行预设库，新增了一批复杂度高的和弦进行预设，并都纳入了对应的分解机制
- 若干性能优化和内存泄漏修复
...
> ![和弦调性主界面png](file://C:\Users\fucha\Documents\Apps\ChordReference\和弦调性主界面.png?msec=1779369497485)
> 
> 和弦调性主界面.png
> 
> ![和弦进行预设系统png](file://C:\Users\fucha\Documents\Apps\ChordReference\和弦进行预设系统.png?msec=1779369527136)和弦进行预设系统.png
> 
> ![分解范式分类库png](file://C:\Users\fucha\Documents\Apps\ChordReference\分解范式分类库.png?msec=1779370520081)分解范式分类库.png
